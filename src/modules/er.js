    ...
    if (selData.length === 1 && selData[0].type === 'entity') {
      ...
      propsEl.innerHTML =
        ...
        P.actionBarHtml('sel-ent', {
          ...
          // できないことを**言う** (UI-049)。classDiagram では ↑↓ が出るのに
          // こちらでは出ない差が、画面から読めなかった。無いものは探しても
          // 見つからないので、探す時間がそのまま失われる。
          //
          // 理由は**文書の形で変わる**。最初に書いた理由 (「行が関係行を指すため」)
          // はひな形だけを測ったもので、宣言行を持つ文書では成り立たなかった。
          //
          // 実測:
          //   宣言が連続している文書   → A,B,C を B,A,C に**正しく並べ替える**
          //   関係行が宣言の間にある   → 本文は動くが**一覧の並びは変わらない**
          //     (エンティティの順序は「最初に現れた行」で決まり、
          //      関係行が宣言より前にあるとその行が順序を決めてしまう)
          //
          // どちらになるかを押す前に見分けられないので、UI からは出さない。
          moveDisabledReason: 'エンティティの順序は最初に現れた行で決まります。関係行が宣言より前にあると、動かしても順序が変わりません',
