    ...
    if (!selData || selData.length === 0) {
      ...
      function doAddTask() {
        ...
        if (!start) {
          ...
          // 欄名と同じ語で言う。マイルストーンのときは欄が「日付」なのに警告だけ
          // 「**開始日**を入れてください」と出ており、画面のどこにも無い欄を
          // 探させていた。マイルストーンは点なので「バーを置く位置」も当たらない。
          ctx.showTransient(milestone
            ? '日付を入れてください — マイルストーンを置く位置が決まりません'
            : '開始日を入れてください — 日付が無いとバーを置く位置が決まりません', 4000);
          return;
        }
