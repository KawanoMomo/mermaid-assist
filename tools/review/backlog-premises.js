...
const SINCE = {
  // 期限が無い唯一の項目だった。33サイクル滞留していながら、
  // 期限が無いので N4 は「まだ待てる」と数え続けてしまう。
  // 待ち続けることと管理することは違うので、他と同じ扱いにした。
  // ラウンド59 で決着 (class=実装 / er・state=現状維持)。滞留監視から外す。
  // 一律に決めるのが誤りで、測ったら class だけ条件が揃っていた。
  // ラウンド59 で期限が到来し、予告どおり H区分へ凍結した。
  // 凍結後は「人の決定待ち」ではなくなるので、滞留の監視対象から外す。
  // 復活条件 (棚卸しに「採用: A」と書く) は C60 の削除リスト監視が見ている。
};
